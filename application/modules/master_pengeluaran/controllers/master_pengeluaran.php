<?php 
class master_pengeluaran extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("keluarm","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Data Pengeluaran";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pengeluaran"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_pengeluaran']	= $result[$i]['id_pengeluaran'] ;
            $responce->rows[$i]['pengeluaran']      = $result[$i]['pengeluaran'] ;
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("master_pengeluaran",$data);
...
function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pengeluaran']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("master_pengeluaran",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("master_pengeluaran",array("deleted"=>1));
