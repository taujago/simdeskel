...
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Aset Sarana Produksi');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Aset Sarana Produksi'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_aset_produksi;
				console.log(row[i].id_aset_produksi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
