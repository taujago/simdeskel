<?php 
class master_produksi_perkebunan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("kebunm","dm");
	}
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"produksi_perkebunan"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_produksi_perkebunan']			= $result[$i]['id_produksi_perkebunan'] ;
            $responce->rows[$i]['produksi_perkebunan']      = $result[$i]['produksi_perkebunan'] ;
    ...
    function update() {
        ...
        $this->form_validation->set_rules('produksi_perkebunan','Produksi Perkebunan ','required');               
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_produksi_perkebunan",$data['id_produksi_perkebunan']);
                $res = $this->db->update("master_produksi_perkebunan",$data);
...
function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_produksi_perkebunan']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("master_produksi_perkebunan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_produksi_perkebunan",$id);
        $this->db->update("master_produksi_perkebunan",array("deleted"=>1));
