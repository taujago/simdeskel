...
$(".easyui-linkbutton").click(function(){
						$("#alat_produksi_ikan").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Alat Produksi Budidaya Ikan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Alat Produksi Budidaya Ikan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_alat_produksi_ikan;
				console.log(row[i].id_alat_produksi_ikan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
