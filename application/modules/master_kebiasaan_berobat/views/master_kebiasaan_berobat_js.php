...
$(".easyui-linkbutton").click(function(){
						$("#kebiasaan_berobat").focus();
						});
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kebiasaan Berobat');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kebiasaan Berobat'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kebiasaan_berobat;
				console.log(row[i].id_kebiasaan_berobat);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
