<?php 
class master_sektor extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("msek","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Sektor pekerjaan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_sektor"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_sektor']			= $result[$i]['id_sektor'] ;
            $responce->rows[$i]['sektor']           = $result[$i]['sektor'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('sektor','Sektor pekerjaan ','required');    
...
//				$data[''] = 
                ...
                $res = $this->db->insert("master_sektor",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_",$data['id_']);
                $res = $this->db->update("master_sektor",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("master_sektor",array("deleted"=>1));
    }
