<?php 
class master_aset_tanah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mptk","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Penguasaan Aset Tanah oleh Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"aset_tanah"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_aset_tanah']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_penguasaan_tanah']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_aset_tanah']			= $result[$i]['id_aset_tanah'] ;
            $responce->rows[$i]['aset_tanah']           = $result[$i]['aset_tanah'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('aset_tanah','Penguasaan Aset Tanah ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_aset_tanah']);
//				$data['penguasaan_tanah'] = 
					
                $res = $this->db->insert("master_aset_tanah",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_aset_tanah",$data['id_aset_tanah']);
                $res = $this->db->update("master_aset_tanah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("master_aset_tanah",array("deleted"=>1));
    }
