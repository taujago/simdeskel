...
$(".easyui-linkbutton").click(function(){
						$("#penguasaan_tanah").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Penguasaan Aset Tanah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Penguasaan Aset Tanah'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_aset_tanah;
				console.log(row[i].id_aset_tanah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
