...
$(".easyui-linkbutton").click(function(){
						$("#kekerasan_rumah").focus();
						});
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kekerasan dalam Keluarga');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kekerasan dalam Keluarga'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kekerasan_rumah;
				console.log(row[i].id_kekerasan_rumah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
