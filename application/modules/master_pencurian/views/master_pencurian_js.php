...
function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Pencurian');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Pencurian'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_pencurian;
				console.log(row[i].id_pencurian);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
