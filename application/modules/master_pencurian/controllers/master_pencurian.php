<?php 
class master_pencurian extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpc","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pencurian";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pencurian"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_pencurian']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_pencurian']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_pencurian']			= $result[$i]['id_pencurian'] ;
            $responce->rows[$i]['pencurian']           = $result[$i]['pencurian'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pencurian']);
//				$data['pencurian'] = 
					
                $res = $this->db->insert("master_pencurian",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_pencurian",$data['id_pencurian']);
                $res = $this->db->update("master_pencurian",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_pencurian",$id);
        $this->db->update("master_pencurian",array("deleted"=>1));
    }
