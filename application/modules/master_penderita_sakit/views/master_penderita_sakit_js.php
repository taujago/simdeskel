...
$(".easyui-linkbutton").click(function(){
						$("#penderita_sakit").focus();
						});
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Penderita Sakit dan Kelainan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Penderita Sakit dan Kelainan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_penderita_sakit;
				console.log(row[i].id_penderita_sakit);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
