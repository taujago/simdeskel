<?php 
class statistik_penduduk_datang extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("pendm","dm");
...
function index()
    ...
    $data['header'] = "Data Penduduk Pendatang";
	$data['controller'] = "statistik_penduduk_datang";
...
function get_data(){
 ...
 function cetak() {
	...
	$this->load->view("statistik_penduduk_datang_cetak",$data); 	
   ...
   $this->db->select('*')->from('v_penduduk p')
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_datang_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_datang_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
