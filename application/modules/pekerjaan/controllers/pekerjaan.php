<?php 
class pekerjaan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpekerjaan","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pekerjaan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pekerjaan"; // get index row - i.e. user click to sort 
	...
	function save(){
		...
		$this->form_validation->set_rules('id_sektor','Sektor pekerjaan ','required');
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pekerjaan']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("pekerjaan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("pekerjaan",array("deleted"=>1));
