...
$(".easyui-linkbutton").click(function(){
						$("#kejahatan_seksual").focus();
						});
var url;

function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kejahatan Seksual');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kejahatan Seksual'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kejahatan_seksual;
				console.log(row[i].id_kejahatan_seksual);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
