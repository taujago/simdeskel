<?php 
class master_kejahatan_seksual extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mks","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Kejahatan Seksual";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"kejahatan_seksual"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_kejahatan_seksual']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_kejahatan_seksual']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_kejahatan_seksual']			= $result[$i]['id_kejahatan_seksual'] ;
            $responce->rows[$i]['kejahatan_seksual']           = $result[$i]['kejahatan_seksual'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_kejahatan_seksual']);
//				$data['kejahatan_seksual'] = 
					
                $res = $this->db->insert("master_kejahatan_seksual",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_kejahatan_seksual",$data['id_kejahatan_seksual']);
                $res = $this->db->update("master_kejahatan_seksual",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_kejahatan_seksual",$id);
        $this->db->update("master_kejahatan_seksual",array("deleted"=>1));
    }
