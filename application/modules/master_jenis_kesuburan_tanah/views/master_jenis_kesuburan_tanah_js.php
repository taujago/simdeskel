...
function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Jenis dan Kesuburan Tanah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Jenis dan Kesuburan Tanah'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_jenis_kesuburan_tanah;
				console.log(row[i].id_jenis_kesuburan_tanah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
