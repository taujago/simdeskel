<?php 
class master_pemasaran_hasil extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mph","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pemasaran hasil";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pemasaran_hasil"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_pemasaran_hasil']			= $result[$i]['id_pemasaran_hasil'] ;
            $responce->rows[$i]['pemasaran_hasil']      = $result[$i]['pemasaran_hasil'] ;
    ...
    function update() {
       ...
       $this->form_validation->set_rules('pemasaran_hasil','Pemasaran hasil ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_pemasaran_hasil",$data['id_pemasaran_hasil']);
                $res = $this->db->update("master_pemasaran_hasil",$data);
...
function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pemasaran_hasil']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("master_pemasaran_hasil",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_pemasaran_hasil",$id);
        $this->db->update("master_pemasaran_hasil",array("deleted"=>1));
