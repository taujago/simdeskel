<?php 
class master_satuan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("ms","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Satuan-satuan yang Dipakai";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"satuan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_satuan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_satuan']=$result[$i]['id_provinsi'];
...
//				$data['satuan'] = 
                ...
                $res = $this->db->insert("master_satuan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("master_satuan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("master_satuan",array("deleted"=>1));
    }
