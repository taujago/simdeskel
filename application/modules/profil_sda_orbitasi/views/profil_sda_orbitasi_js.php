...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Orbitasi');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Orbitasi'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_orbitasi;
				console.log(row[i].id_orbitasi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
