<?php 
class profil_sda_orbitasi extends op_controller {
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_orbitasi"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_orbitasi']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
			if($result[$i]['pernyataan'] == 1){$temp = "Ada"; }
			else if($result[$i]['pernyataan'] == 0 or empty($result[$i]['pernyataan'])){$temp = ""; }
			else{$temp = "Tidak"; }
            //$responce->rows[$i]['id_topografi']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_orbitasi']			= $result[$i]['id_orbitasi'] ;
            $responce->rows[$i]['orbitasi']           = $result[$i]['orbitasi'] ;
     		$responce->rows[$i]['id_satuan']			= $result[$i]['id_satuan'] ;
			$responce->rows[$i]['nilai']			= $result[$i]['nilai'] ;
            $responce->rows[$i]['satuan']           = $result[$i]['satuan'] ;
			$responce->rows[$i]['pernyataan']           = $result[$i]['pernyataan'] ;
			$responce->rows[$i]['pernyataan1']           = $temp;
        } }
		//echo "<hr />";
    ...
    function save(){
        ...
        $this->form_validation->set_rules('orbitasi','Orbitasi ','required');   
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_orbitasi']);
//				$data['topografi'] = 
					
                $res = $this->db->insert("profil_sda_orbitasi",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_orbitasi",$data['id_orbitasi']);
                $res = $this->db->update("profil_sda_orbitasi",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_orbitasi",$id);
        $this->db->update("profil_sda_orbitasi",array("deleted"=>1));
    }
