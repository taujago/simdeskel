...
$(".easyui-linkbutton").click(function(){
						$("#produksi_perikanan").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Produksi Perikanan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Produksi Perikanan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_produksi_perikanan;
				console.log(row[i].id_produksi_perikanan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
