...
function baru(id){
			...
			var lef = (wd - 600)/2;
			tp = tp + 150;
			$('#dlg_' + id).dialog('move',{left:lef,top:tp});
...
function reload(id){
	$.post('<?php echo site_url("perkembangan_jumlah_penduduk/reload") ?>',function(result){
...
function edit(id) {
	...
	$('#dlg_' + id).dialog('move',{left:lef,top:tp});
	if(row) { 
	$('#dlg_' + id).dialog('open').dialog('setTitle','Edit Data '+title); 
