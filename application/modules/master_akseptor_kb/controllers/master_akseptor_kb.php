<?php 
class master_akseptor_kb extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mak","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Akseptor KB";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"akseptor_kb"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_akseptor_kb']			= $result[$i]['id_akseptor_kb'] ;
            $responce->rows[$i]['akseptor_kb']      = $result[$i]['akseptor_kb'] ;
    ...
    function update() {
        ...
        $this->form_validation->set_rules('akseptor_kb','Akseptor KB ','required');               
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_akseptor_kb",$data['id_akseptor_kb']);
                $res = $this->db->update("master_akseptor_kb",$data);
...
//				$data['hubungan'] = 
                ...
                $res = $this->db->insert("master_akseptor_kb",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_akseptor_kb",$id);
        $this->db->update("master_akseptor_kb",array("deleted"=>1));
