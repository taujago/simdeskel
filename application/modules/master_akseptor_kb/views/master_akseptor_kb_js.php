...
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Akseptor KB');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Akseptor KB'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_akseptor_kb;
				console.log(row[i].id_akseptor_kb);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
