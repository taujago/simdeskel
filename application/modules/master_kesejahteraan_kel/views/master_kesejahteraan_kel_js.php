...
$(".easyui-linkbutton").click(function(){
						$("#kesejahteraan_kel").focus();
						});
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Masalah Kesejahteraan Keluarga');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Masalah Kesejahteraan Keluarga'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kesejahteraan_kel;
				console.log(row[i].id_kesejahteraan_kel);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
