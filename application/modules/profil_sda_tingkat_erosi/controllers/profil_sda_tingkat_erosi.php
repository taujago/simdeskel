<?php 
class profil_sda_tingkat_erosi extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("pste","dm");
	}
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_tingkat_erosi"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_tingkat_erosi']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_tingkat_erosi']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_tingkat_erosi']			= $result[$i]['id_tingkat_erosi'] ;
            $responce->rows[$i]['tingkat_erosi']           = $result[$i]['tingkat_erosi'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('tingkat_erosi','Tingkat Erosi Tanah ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_tingkat_erosi']);
//				$data['tingkat_erosi'] = 
					
                $res = $this->db->insert("profil_sda_tingkat_erosi",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_tingkat_erosi",$data['id_tingkat_erosi']);
                $res = $this->db->update("profil_sda_tingkat_erosi",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_tingkat_erosi",$id);
        $this->db->update("profil_sda_tingkat_erosi",array("deleted"=>1));
    }
