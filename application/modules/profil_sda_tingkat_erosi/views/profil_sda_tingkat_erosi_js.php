...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tingkat Erosi Tanah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tingkat Erosi Tanah'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tingkat_erosi;
				console.log(row[i].id_tingkat_erosi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
