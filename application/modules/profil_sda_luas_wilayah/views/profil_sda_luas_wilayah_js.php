...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Luas Wilayah Menurut Penggunaan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Luas Wilayah Menurut Penggunaan'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_luas_wilayah;
				console.log(row[i].id_luas_wilayah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
