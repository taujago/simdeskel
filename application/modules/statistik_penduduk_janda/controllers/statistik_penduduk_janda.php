<?php 
class statistik_penduduk_janda extends  op_controller {
...
function index()
    ...
    $data['title'] = "Data Penduduk  Janda";	
    $data['header'] = "Data Penduduk Janda";
	$data['controller'] = "statistik_penduduk_janda";
   	 

   	$content = $this->load->view("statistik_penduduk_janda_view",$data,true);
...
function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
			...
			$responce->rows[$i]['foto2']			= (!empty($result[$i]['foto']))?
			'<img src="'.base_url().'foto/' .$result[$i]['foto'].'" />':
			'<img src="'.base_url().'foto/no_photo.jpg" />';
 ...
 function cetak() {
 	$this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
		...
		where("status_kawin",'j')
	->order_by("dusun,rw,rt,nama");

 	$data['record'] = $res = $this->db->get('v_penduduk p');
	$id_desa =	 $this->session->userdata("operator_id_desa");
	...
	$this->load->view("statistik_penduduk_janda_cetak",$data); 	
  ...
  $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_janda_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_janda_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
