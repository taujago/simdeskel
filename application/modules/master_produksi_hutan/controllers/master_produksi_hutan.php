<?php 
class master_produksi_hutan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mphkel","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Produksi Hasil Hutan Milik Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"produksi_hutan"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_produksi_hutan']			= $result[$i]['id_produksi_hutan'] ;
			 $responce->rows[$i]['produksi_hutan']      = $result[$i]['produksi_hutan'] ;
    ...
    function update() {
        ...
        $this->form_validation->set_rules('produksi_hutan','Produksi hasil hutan ','required');  
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_produksi_hutan",$data['id_produksi_hutan']);
                $res = $this->db->update("master_produksi_hutan",$data);
...
function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_produksi_hutan']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("master_produksi_hutan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_produksi_hutan",$id);
        $this->db->update("master_produksi_hutan",array("deleted"=>1));
