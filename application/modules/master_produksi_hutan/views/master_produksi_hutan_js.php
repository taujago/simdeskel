...
$(".easyui-linkbutton").click(function(){
						$("#produksi_hutan").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Produksi Hasil Hutan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Produksi Hasil Hutan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_produksi_hutan;
				console.log(row[i].id_produksi_hutan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
