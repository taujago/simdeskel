<?php 
class profil_sda_letak extends op_controller {
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_letak"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_letak']			= $result[$i]['id_letak'] ;
            $responce->rows[$i]['letak']           = $result[$i]['letak'] ;
...
//				$data['topografi'] = 
                ...
                $res = $this->db->insert("profil_sda_letak",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_letak",$data['id_letak']);
                $res = $this->db->update("profil_sda_letak",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("profil_sda_letak",array("deleted"=>1));
    }
