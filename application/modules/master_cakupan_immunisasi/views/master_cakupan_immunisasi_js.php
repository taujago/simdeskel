...
$(".easyui-linkbutton").click(function(){
						$("#cakupan_immunisasi").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Cakupan Immunisasi');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Cakupan Immunisasi'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_cakupan_immunisasi;
				console.log(row[i].id_cakupan_immunisasi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
