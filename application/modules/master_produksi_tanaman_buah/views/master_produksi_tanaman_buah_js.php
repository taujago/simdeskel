...
$(".easyui-linkbutton").click(function(){
						$("#produksi_tanaman_buah").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Produksi tanaman buah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Produksi tanaman buah'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				...
				console.log(row[i].id_produksi_tanaman_buah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
