...
$(".easyui-linkbutton").click(function(){
						$("#pengolahan_ternak").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Usaha Pengolahan Hasil Ternak');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Usaha Pengolahan Hasil Ternak'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_pengolahan_ternak;
				console.log(row[i].id_pengolahan_ternak);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
