...
$(".easyui-linkbutton").click(function(){
						$("#lembaga_pendidikan").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Lembaga Pendidikan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Lembaga Pendidikan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_lembaga_pendidikan;
				console.log(row[i].id_lembaga_pendidikan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
