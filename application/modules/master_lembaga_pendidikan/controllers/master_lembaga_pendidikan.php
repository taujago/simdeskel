<?php 
class master_lembaga_pendidikan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mlpk","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Lembaga Pendidikan yang Dimiliki Keluarga/Komunitas";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"lembaga_pendidikan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_lembaga_pendidikan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_lembaga_pendidikan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_lembaga_pendidikan']			= $result[$i]['id_lembaga_pendidikan'] ;
            $responce->rows[$i]['lembaga_pendidikan']           = $result[$i]['lembaga_pendidikan'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_lembaga_pendidikan']);
//				$data['lembaga_pendidikan'] = 
					
                $res = $this->db->insert("master_lembaga_pendidikan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_lembaga_pendidikan",$data['id_lembaga_pendidikan']);
                $res = $this->db->update("master_lembaga_pendidikan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_lembaga_pendidikan",$id);
        $this->db->update("master_lembaga_pendidikan",array("deleted"=>1));
    }
