...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Bentangan wilayah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Bentangan wilayah'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_topografi;
				console.log(row[i].id_topografi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
