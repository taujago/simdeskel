...
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Status Gizi Balita');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Status Gizi Balita'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_status_gizi;
				console.log(row[i].id_status_gizi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
