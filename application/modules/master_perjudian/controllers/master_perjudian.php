<?php 
class master_perjudian extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mper","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Perjudian";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"perjudian"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_perjudian']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_perjudian']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_perjudian']			= $result[$i]['id_perjudian'] ;
            $responce->rows[$i]['perjudian']           = $result[$i]['perjudian'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('perjudian','Perjudian ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_perjudian']);
//				$data['perjudian'] = 
					
                $res = $this->db->insert("master_perjudian",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_perjudian",$data['id_perjudian']);
                $res = $this->db->update("master_perjudian",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_perjudian",$id);
        $this->db->update("master_perjudian",array("deleted"=>1));
    }
