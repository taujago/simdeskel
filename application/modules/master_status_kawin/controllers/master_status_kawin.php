<?php 
class master_status_kawin extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("msk","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Status perkawinan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"stat"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['stat']      = $result[$i]['stat'] ;
    ...
    function update() {
        ...
        $this->form_validation->set_rules('stat','Status hubungan','required');               
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("master_status_kawin",$data);
