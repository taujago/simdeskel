<?php 
class profil_tanah extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("ptm","dm");
	...
	function index(){
		...
		$data['header'] = "Data Tanah " . $desa2->bentuk_lembaga;
	...
	function get_data(){
       ...
       /*$nama_ibu = isset($_REQUEST['search_nama_ibu'])?$_REQUEST['search_nama_ibu']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['asal']                 = $result[$i]['asal'] ;
            $responce->rows[$i]['no_sertifikat']        = $result[$i]['no_sertifikat'] ;
            $responce->rows[$i]['luas']                 = $result[$i]['luas'] ;
            $responce->rows[$i]['kelas']                = $result[$i]['kelas'] ;
            $responce->rows[$i]['tkd_id']               = $result[$i]['tkd_id'] ;
            $responce->rows[$i]['tkd']                  = $result[$i]['tkd'] ;
            $responce->rows[$i]['tkd_tgl']              = flipdate($result[$i]['tkd_tgl']) ;
            $responce->rows[$i]['tkd_id_jenis']         = $result[$i]['tkd_id_jenis'] ;
            $responce->rows[$i]['tkd_jenis']            = $result[$i]['tkd_jenis'] ;

            $responce->rows[$i]['patok']         = $result[$i]['patok'] ;
            $responce->rows[$i]['papan_nama']            = $result[$i]['papan_nama'] ;

            $responce->rows[$i]['patok2']                = $arr_ada[$result[$i]['patok']] ;
            $responce->rows[$i]['papan_nama2']           = $arr_ada[$result[$i]['papan_nama']] ;
