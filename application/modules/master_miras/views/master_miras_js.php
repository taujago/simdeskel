...
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Pemakaian MIRAS dan Narkoba');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Pemakaian MIRAS dan Narkoba'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_miras;
				console.log(row[i].id_miras);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
