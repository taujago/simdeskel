<?php 
class master_miras extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mm","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pemakaian MIRAS dan Narkoba";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"miras"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_miras']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_miras']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_miras']			= $result[$i]['id_miras'] ;
            $responce->rows[$i]['miras']           = $result[$i]['miras'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('miras','Pemakaian MIRAS dan Narkoba ','required');    
...
//				$data['miras'] = 
                ...
                $res = $this->db->insert("master_miras",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_miras",$data['id_miras']);
                $res = $this->db->update("master_miras",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_miras",$id);
        $this->db->update("master_miras",array("deleted"=>1));
    }
