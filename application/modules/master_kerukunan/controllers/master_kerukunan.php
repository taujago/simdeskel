<?php 
class master_kerukunan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mk","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Kerukunan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"kerukunan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_kerukunan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_kerukunan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_kerukunan']			= $result[$i]['id_kerukunan'] ;
            $responce->rows[$i]['kerukunan']           = $result[$i]['kerukunan'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('kerukunan','Kerukunan ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_kerukunan']);
//				$data['kerukunan'] = 
					
                $res = $this->db->insert("master_kerukunan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_kerukunan",$data['id_kerukunan']);
                $res = $this->db->update("master_kerukunan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_kerukunan",$id);
        $this->db->update("master_kerukunan",array("deleted"=>1));
    }
