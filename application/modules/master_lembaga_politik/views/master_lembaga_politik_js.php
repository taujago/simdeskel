...
$(".easyui-linkbutton").click(function(){
						$("#lembaga_politik").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Lembaga');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Lembaga'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_lembaga_politik;
				console.log(row[i].id_lembaga_politik);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
