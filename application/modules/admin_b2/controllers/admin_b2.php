<?php 
class admin_b2 extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("b2m","dm");
	...
	function index(){
		...
		$data['header'] = "BUKU MUTASI PENDUDUK  " . strtoupper($desa2->bentuk_lembaga);
...
function cetak($tahun) {
    ...
    $data['tahun'] = $tahun;
    $data['header'] = "Buku Data Mutasi Penduduk ".$this->desa2->bentuk_lembaga;
...
function pdf($tahun){
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/admin_b2_pdf_view",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/admin_b2_pdf_view_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
function excel($tahun) {
    ...
    $this->load->library('Excel');
        ...
        $this->excel->getActiveSheet()->setTitle('buku mutasi penduduk ');
...
$this->excel->getActiveSheet()->mergeCells('K'.$baris.':N'.$baris);
...
$this->excel->getActiveSheet()
        ...
        ->setCellValue('C' . $baris, "TEMPAT DAN TANGGAL LAHIR ")
        ->setCellValue('C' . ($baris+1), "TEMPAT LAHIR")
        ->setCellValue('D' . ($baris+1), "TANGGAL LAHIR")
        ->setCellValue('E' . $baris, "JK")
        ->setCellValue('F' . $baris, "KEWARGANEGARAAN")     
        ->setCellValue('G' . ($baris+1), "DATANG DARI")
        ->setCellValue('H' . ($baris+1), "TANGGAL")
        ->setCellValue('I' . ($baris+1), "LAHIR")
        ->setCellValue('J' . ($baris+1), "TANGGAL")
        ->setCellValue('K' . ($baris+1), "PINDAH KE")
        ->setCellValue('L' . ($baris+1), "TANGGAL")
        ->setCellValue('M' . ($baris+1), "MATI")
        ->setCellValue('N' . ($baris+1), "TANGGAL")
        
        ->setCellValue('G' . $baris, "PENAMBAHAN")
        ->setCellValue('K' . $baris, "PENGELUARAN")
...
foreach($record->result() as $row) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('C' . $baris, $row->tmp_lahir)
        ->setCellValue('D' . $baris, flipdate($row->tgl_lahir))
        ->setCellValue('E' . $baris, $row->jk)
        ->setCellValue('F' . $baris, $row->warga_negara)
        ->setCellValue('G' . $baris, $row->datang_dari)
        ->setCellValue('H' . $baris, $row->datang_tanggal)
        ->setCellValue('I' . $baris, $row->tmp_dilahirkan)
        ->setCellValue('J' . $baris, flipdate($row->tgl_lahir2))
        ->setCellValue('K' . $baris, $row->pindah_ke)
        ->setCellValue('L' . $baris, flipdate($row->pindah_tanggal) )
        ->setCellValue('M' . $baris, $row->mati)
        ->setCellValue('N' . $baris, flipdate($row->mati_tanggal)  );
...
->setCellValue('O' . $baris, "Pangkat  : " . $this->desa2->pangkat_sek_desa);
         ...
         $filename = "BUKU DATA MUTASI PENDUDUK  ".strtoupper($this->desa2->bentuk_lembaga).".xls";
