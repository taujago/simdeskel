<?php 
class admin_a2 extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("a2m","dm");
	...
	function index(){
		...
		$data['header'] = "Data Keputusan Kepala  " . $desa2->bentuk_lembaga;
...
/*            $responce->rows[$i]['persetujuan_bpd_nomor']           = $result[$i]['persetujuan_bpd_nomor'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
              ...
              //  $data['persetujuan_bpd_tgl'] = flipdate($data['persetujuan_bpd_tgl']);
                ...
                $res = $this->db->insert("admin_a2",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("admin_a2",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("admin_a2",array("deleted"=>1));
    }
...
function pdf($tahun){
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/admin_a2_pdf_view",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
           ...
           // $html.="<h1> beda page </h1>";
            $pdf->AddPage();
            $html = $this->load->view("pdf/admin_a2_pdf_view_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
function excel($tahun) {
        ...
        ->setCellValue('A' . $baris,"BUKU KEPUTUSAN KEPALA  ". strtoupper($this->desa2->bentuk_lembaga));
        $baris++;
...
$this->excel->getActiveSheet()
        ...
        ->setCellValue('D' . $baris, "URAIAN SINGKAT ")
         ->setCellValue('e' . $baris, "NOMOR DAN TANGGAL DILAPORKAN")
...
foreach($record->result() as $row) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('D' . $baris, $row->uraian_singkat)
         ->setCellValue('E' . $baris,"Nomor :  $row->dilaporkan_nomor \nTanggal : $row->dilaporkan_tgl")
