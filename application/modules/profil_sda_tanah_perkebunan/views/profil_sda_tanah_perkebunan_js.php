...
function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tanah Perkebunan');
			$('#fm').form('clear');
			url = '<?php echo site_url("$controller/save") ?>'; 
			$("#tanah_perkebunan").focus();
}
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tanah Perkebunan'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tanah_perkebunan;
				console.log(row[i].id_tanah_perkebunan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
