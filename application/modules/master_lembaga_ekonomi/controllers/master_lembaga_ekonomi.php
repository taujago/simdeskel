<?php 
class master_lembaga_ekonomi extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mle","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Lembaga Ekonomi yang Dimiliki Anggota Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"lembaga_ekonomi"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_lembaga_ekonomi']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_lembaga_ekonomi']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_lembaga_ekonomi']			= $result[$i]['id_lembaga_ekonomi'] ;
            $responce->rows[$i]['lembaga_ekonomi']           = $result[$i]['lembaga_ekonomi'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_lembaga_ekonomi']);
//				$data['lembaga_ekonomi'] = 
					
                $res = $this->db->insert("master_lembaga_ekonomi",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_lembaga_ekonomi",$data['id_lembaga_ekonomi']);
                $res = $this->db->update("master_lembaga_ekonomi",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_lembaga_ekonomi",$id);
        $this->db->update("master_lembaga_ekonomi",array("deleted"=>1));
    }
