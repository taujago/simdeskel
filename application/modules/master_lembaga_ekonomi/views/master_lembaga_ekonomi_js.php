...
$(".easyui-linkbutton").click(function(){
						$("#lembaga_ekonomi").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Lembaga Ekonomi');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Lembaga Ekonomi'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_lembaga_ekonomi;
				console.log(row[i].id_lembaga_ekonomi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
