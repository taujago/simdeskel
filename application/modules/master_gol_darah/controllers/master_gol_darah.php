<?php 
class master_gol_darah extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mgd","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Golongan darah";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"gol"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['gol']      = $result[$i]['gol'] ;
...
//				$data['hubungan'] = 
                ...
                $res = $this->db->insert("master_gol_darah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("master_gol_darah",array("deleted"=>1));
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("master_gol_darah",$data);
