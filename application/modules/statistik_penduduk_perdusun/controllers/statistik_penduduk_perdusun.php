<?php 
class statistik_penduduk_perdusun extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("stat_ppd","dm");
...
function index()
    ...
    $data['title'] = "Data Penduduk Per Dusun ";	
    $data['header'] = "Data Penduduk Per Dusun";
	$data['controller'] = "statistik_penduduk_perdusun";
  

 
 
   	$content = $this->load->view("statistik_penduduk_perdusun_view",$data,true);
...
function get_data(){
       ...
       /* $nama  = isset($_REQUEST['search_nama'])?$_REQUEST['search_nama']:"x";
        ...
        $req_param = array (
				...
				"sort_direction" => $sord,
				"limit" => null  ,
				"id_dusun" => isset($_REQUEST['id_dusun'])?$_REQUEST['id_dusun']:"x",
 ...
 function cetak($id_dusun) {
	...
	$data['rec_dusun'] = $this->db->get("tiger_dusun");
	//echo $this->db->last_query();

	$this->load->view("statistik_penduduk_perdusun_cetak",$data); 	
  ...
  function pdf($id_dusun){

  	$id_dusun = str_replace("%20"," ",$id_dusun);

  	$this->db->select('distinct dusun',false)->from("penduduk");
   	if($id_dusun <> 'x'){
		$this->db->where("dusun",$id_dusun);
	}
	 
	$this->db->order_by("dusun");
	$data['rec_dusun'] = $this->db->get();
	// echo $this->db->last_query(); 
	// exit;
    $data['header'] = "DATA PENDUDUK PER DUSUN ".$this->desa2->bentuk_lembaga;
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_perdusun_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_perdusun_pdf_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
