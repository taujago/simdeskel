...
$(".easyui-linkbutton").click(function(){
						$("#produksi_tanaman_obat").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Produksi tanaman obat');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Produksi tanaman obat'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_produksi_tanaman_obat;
				console.log(row[i].id_produksi_tanaman_obat);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
