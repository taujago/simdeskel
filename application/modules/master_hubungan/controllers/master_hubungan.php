<?php 
class master_hubungan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mhn","dm");
	}
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"hubungan"; // get index row - i.e. user click to sort 
    ...
    function save(){
        ...
        $this->form_validation->set_rules('hubungan','Nama hubungan','required');    
...
//				$data['hubungan'] = 
                ...
                $res = $this->db->insert("master_hubungan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("master_hubungan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("master_hubungan",array("deleted"=>1));
    }
