...
$(".easyui-linkbutton").click(function(){
						$("#kualitas_hamil").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kualitas Ibu Hamil');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kualitas Ibu Hamil'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kualitas_hamil;
				console.log(row[i].id_kualitas_hamil);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
