<?php 
class master_sumber_air extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("msa","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Sumber Air Minum yang Digunakan Anggota Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"sumber_air"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_sumber_air']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_sumber_air']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_sumber_air']			= $result[$i]['id_sumber_air'] ;
            $responce->rows[$i]['sumber_air']           = $result[$i]['sumber_air'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('sumber_air','Sumber Air Minum ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_sumber_air']);
//				$data['sumber_air'] = 
					
                $res = $this->db->insert("master_sumber_air",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_sumber_air",$data['id_sumber_air']);
                $res = $this->db->update("master_sumber_air",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_sumber_air",$id);
        $this->db->update("master_sumber_air",array("deleted"=>1));
    }
