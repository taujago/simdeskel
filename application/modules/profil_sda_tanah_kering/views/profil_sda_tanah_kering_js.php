...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tanah Kering');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tanah Kering'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tanah_kering;
				console.log(row[i].id_tanah_kering);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
