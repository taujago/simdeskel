...
$('#tanggal, #tanggal_surat_masuk, #tanggal_surat_keluar, #search_tgl_awal').datebox({  
        required:false  ,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Agenda BPD ');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Agenda BPD '); 
	$('#fm').form('load',row);						 
