...
$(".easyui-linkbutton").click(function(){
						$("#aset_transportasi").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Aset Sarana Transportasi Umum');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Aset Sarana Transportasi Umum'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_aset_transportasi;
				console.log(row[i].id_aset_transportasi);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
