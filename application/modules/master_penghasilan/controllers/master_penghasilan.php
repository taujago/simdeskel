<?php 
class master_penghasilan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("hasilm","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Data Penghasilan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"penghasilan"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['id_penghasilan']	= $result[$i]['id_penghasilan'] ;
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_penghasilan",$data['id_penghasilan']);
                $res = $this->db->update("master_penghasilan",$data);
...
function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_penghasilan']);
//				$data['hubungan'] = 
					
                $res = $this->db->insert("master_penghasilan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("master_penghasilan",array("deleted"=>1));
