<?php 
class statistik_penduduk_pemilu extends  op_controller {
...
function index()
    ...
    $data['title'] = "Data Penduduk Memiliki Hak Pilih";	
    $data['header'] = "Data Penduduk Memiliki Hak Pilih";
	$data['controller'] = "statistik_penduduk_pemilu";
    

   	$content = $this->load->view("statistik_penduduk_pemilu_view",$data,true);
...
function get_data(){
        ...
        $sord = isset($_REQUEST['order'])?$_REQUEST['order']:"asc"; // get the  
 ...
 function cetak() {
	...
	$this->load->view("statistik_penduduk_pemilu_cetak",$data); 	
 ...
 function pdf(){
   $this->db->where('p.id_desa',$this->session->userdata("operator_id_desa"));
    ...
    $data['header'] = "DATA PENDUDUK PEMILIH  ";
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_pemilu_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_pemilu_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
