<?php 
class profil_sda_jenis_kesuburan_tanah extends op_controller {
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_jenis_kesuburan_tanah"; // get index row - i.e. user click to sort 
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['jenis_kesuburan_tanah']           = $result[$i]['jenis_kesuburan_tanah'] ;
     		$responce->rows[$i]['nilai']           = !empty($result[$i]['nilai'])?$result[$i]['nilai']:"" ;
			$responce->rows[$i]['id_satuan']           = $result[$i]['id_satuan'];
...
//				$data[''] = 
                ...
                $res = $this->db->insert("profil_sda_jenis_kesuburan_tanah",$data);
    ...
    function update() {
        $data = $this->input->post();
        $this->load->library('form_validation');
        $this->form_validation->set_rules('','Jenis dan Kesuburan Tanah ','required');   
		$this->form_validation->set_rules('id_satuan','Satuan ','required');    
        $this->form_validation->set_message('required', ' %s Harus diisi '); 
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("profil_sda_jenis_kesuburan_tanah",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        ...
        $this->db->update("profil_sda_jenis_kesuburan_tanah",array("deleted"=>1));
    }
