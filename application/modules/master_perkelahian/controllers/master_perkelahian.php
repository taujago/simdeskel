<?php 
class master_perkelahian extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mp","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Perkelahian";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"perkelahian"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_perkelahian']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_perkelahian']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_perkelahian']			= $result[$i]['id_perkelahian'] ;
            $responce->rows[$i]['perkelahian']           = $result[$i]['perkelahian'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('perkelahian','Master Perkelahian ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_perkelahian']);
//				$data['perkelahian'] = 
					
                $res = $this->db->insert("master_perkelahian",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_perkelahian",$data['id_perkelahian']);
                $res = $this->db->update("master_perkelahian",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_perkelahian",$id);
        $this->db->update("master_perkelahian",array("deleted"=>1));
    }
