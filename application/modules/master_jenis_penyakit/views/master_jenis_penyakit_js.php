...
$(".easyui-linkbutton").click(function(){
						$("#jenis_penyakit").focus();
						});
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Jenis Penyakit');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Jenis Penyakit'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_jenis_penyakit;
				console.log(row[i].id_jenis_penyakit);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
