<?php 
class master_jenis_penyakit extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mjp","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Jenis Penyakit yang Diderita Anggota Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"jenis_penyakit"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_jenis_penyakit']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_jenis_penyakit']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_jenis_penyakit']			= $result[$i]['id_jenis_penyakit'] ;
            $responce->rows[$i]['jenis_penyakit']           = $result[$i]['jenis_penyakit'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_jenis_penyakit']);
//				$data['jenis_penyakit'] = 
					
                $res = $this->db->insert("master_jenis_penyakit",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_jenis_penyakit",$data['id_jenis_penyakit']);
                $res = $this->db->update("master_jenis_penyakit",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_jenis_penyakit",$id);
        $this->db->update("master_jenis_penyakit",array("deleted"=>1));
    }
