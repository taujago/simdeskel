...
function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Pembunuhan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Pembunuhan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_pembunuhan;
				console.log(row[i].id_pembunuhan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
