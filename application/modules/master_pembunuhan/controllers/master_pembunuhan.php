<?php 
class master_pembunuhan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpem","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pembunuhan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pembunuhan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_pembunuhan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_pembunuhan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_pembunuhan']			= $result[$i]['id_pembunuhan'] ;
            $responce->rows[$i]['pembunuhan']           = $result[$i]['pembunuhan'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pembunuhan']);
//				$data['pembunuhan'] = 
					
                $res = $this->db->insert("master_pembunuhan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_pembunuhan",$data['id_pembunuhan']);
                $res = $this->db->update("master_pembunuhan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_pembunuhan",$id);
        $this->db->update("master_pembunuhan",array("deleted"=>1));
    }
