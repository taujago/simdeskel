...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data <?php echo $dusun; ?>');
			$('#fm').form('clear');
			$("#id_dusun").val('<?php echo $this->session->userdata("operator_id_desa")."."; ?>');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data <?php echo $dusun; ?>'); 
	$('#fm').form('load',row);
