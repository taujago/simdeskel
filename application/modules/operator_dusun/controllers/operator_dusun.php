<?php 
class operator_dusun extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("op_dusunm","dm");
...
function index()
   ...
   * */
	...
	$data['dusun']  = ($desa->bentuk_lembaga == "Kelurahan")?"Lingkungan":"Dusun";
    $data['header'] = "Data ". $data['dusun'];
	$data['controller'] = "operator_dusun";
...
function update(){
		...
		$this->db->where("id_dusun",$data['id_dusun']);
		$res  = $this->db->update("tiger_dusun",$data);
...
function hapus() {
		...
		foreach($arr_id as $id) {
			...
			$this->db->where('id_dusun',$id);
			$res = $this->db->delete("tiger_dusun");
			if($res) $a[]=$id;
			else $b[]=$id;
