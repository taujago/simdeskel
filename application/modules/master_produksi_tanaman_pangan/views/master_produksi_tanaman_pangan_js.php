...
$(".easyui-linkbutton").click(function(){
						$("#produksi_tanaman_pangan").focus();
						});
var url;

function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Produksi tanaman pangan ');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Produksi tanaman pangan '); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_produksi_tanaman_pangan;
				console.log(row[i].id_produksi_tanaman_pangan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
