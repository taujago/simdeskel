<?php 
class admin_e3 extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("e3m","dm");
	...
	function get_data(){
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['pelaksana'] 	= $result[$i]['pelaksana'] ;
            $responce->rows[$i]['pokok']   		= $result[$i]['pokok'] ;
            $responce->rows[$i]['hasil']   		= $result[$i]['hasil'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('pokok','Pokok-pokok kegiatan ','required');                                  
        ...
        if($this->form_validation->run() == TRUE) {    
                ...
                $res = $this->db->insert("admin_e3",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {      
                ...
                $res = $this->db->update("admin_e3",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("admin_e3",array("deleted"=>1));
    }
...
function cetak($tahun) {
    ...
    $data['tahun'] = $tahun;
    $data['header'] = "BUKU PERATURAN DATA KEGIATAN BPD ".$this->desa2->bentuk_lembaga;
...
function pdf($tahun){
    ...
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/admin_e3_pdf_view",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/admin_e3_pdf_view_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
...
function excel($tahun) {
        ...
        $this->excel->getActiveSheet()->setTitle('BUKU DATA KEGIATAN BPD');
        ...
        ->setCellValue('A' . $baris,"BUKU DATA KEGIATAN BPD");
        $baris++;
		...
		//$this->excel->getActiveSheet()->mergeCells('A'.$baris.':f'.$baris);
        $this->excel->getActiveSheet()->getStyle('f' . $baris)->applyFromArray(
            array(
            'font' => array(
            'name'         => 'Calibri',
            'bold'         => true,
        ...
        $this->excel->getActiveSheet()
        ->setCellValue('f' . $baris, "MODEL E.3"); 
		$baris +=2;


$this->excel->getActiveSheet()
        ->setCellValue('A' . $baris, "NO")
        ->setCellValue('B' . $baris, "TENTANG")
        ->setCellValue('C' . $baris, "PELAKSANA")
        ->setCellValue('D' . $baris, "POKOK-POKOK KEGIATAN")
        ->setCellValue('E' . $baris, "HASIL KEGIATAN")
...
foreach($record->result() as $row) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('B' . $baris, $row->tentang)
        ->setCellValue('C' . $baris, $row->pelaksana)
        ->setCellValue('D' . $baris, $row->pokok)
        ->setCellValue('E' . $baris, $row->hasil)
...
//$this->format(array("arr_kolom"=>$arr_kolom,"bold"=>true,"baris"=>$baris,"align"=>"center"));
        ...
        $filename = "BUKU DATA KEGIATAN BPD DESA ".$this->desa2->desa.".xls";
