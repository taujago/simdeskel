...
function baru(id){
			...
			url = '<?php echo site_url("operator_penduduk/simpan") ?>';  
}
function edit(id) {
	...
	url = '<?php echo site_url("operator_penduduk/update_det") ?>';  
	}
	else {
			...
			console.log(ids);
  			if (row){
				$.messager.confirm('Konfirmasi','Anda yakin akan menghapus data ? ',function(r){
					if (r){
						$.post('<?php echo site_url("operator_penduduk/hapus_det") ?>' + "/" + table,{ids:ids},function(result){
