<?php 
class profil_sda_iklim extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("iklim","dm");
	}
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"id_iklim"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_iklim']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_iklim']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_iklim']		= $result[$i]['id_iklim'] ;
            $responce->rows[$i]['iklim']    	= $result[$i]['iklim'] ;
...
//				$data['iklim'] = 
                ...
                $res = $this->db->insert("profil_sda_iklim",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_iklim",$data['id_iklim']);
                $res = $this->db->update("profil_sda_iklim",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_iklim",$id);
        $this->db->update("profil_sda_iklim",array("deleted"=>1));
    }
