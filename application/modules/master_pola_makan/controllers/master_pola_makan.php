<?php 
class master_pola_makan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpm","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Pola Makan Keluarga";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"pola_makan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_pola_makan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_pola_makan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_pola_makan']			= $result[$i]['id_pola_makan'] ;
            $responce->rows[$i]['pola_makan']           = $result[$i]['pola_makan'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('pola_makan','Pola Makan Keluarga ','required');    
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_pola_makan']);
//				$data['pola_makan'] = 
					
                $res = $this->db->insert("master_pola_makan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_pola_makan",$data['id_pola_makan']);
                $res = $this->db->update("master_pola_makan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_pola_makan",$id);
        $this->db->update("master_pola_makan",array("deleted"=>1));
    }
