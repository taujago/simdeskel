...
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Pola Makan Keluarga');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Pola Makan Keluarga'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_pola_makan;
				console.log(row[i].id_pola_makan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
