...
$(".easyui-linkbutton").click(function(){
						$("#kualitas_persalinan").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kualitas Persalinan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kualitas Persalinan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kualitas_persalinan;
				console.log(row[i].id_kualitas_persalinan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
