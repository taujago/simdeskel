...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tanah Fasilitas Umum');
			$('#fm').form('clear');
			url = '<?php echo site_url("$controller/save") ?>'; 
			$("#tanah_fasilitas_umum").focus();
}
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tanah Fasilitas Umum'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tanah_fasilitas_umum;
				console.log(row[i].id_tanah_fasilitas_umum);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
