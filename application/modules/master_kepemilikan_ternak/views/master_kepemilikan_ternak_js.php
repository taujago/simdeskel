...
$(".easyui-linkbutton").click(function(){
						$("#kepemilikan_ternak").focus();
						});
var url;

function baru(){
	mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Kepemilikan Jenis Ternak');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Kepemilikan Jenis Ternak'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kepemilikan_ternak;
				console.log(row[i].id_kepemilikan_ternak);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
