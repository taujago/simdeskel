<?php 
class master_penjarahan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpen","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Penjarahan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"penjarahan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_penjarahan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_penjarahan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_penjarahan']			= $result[$i]['id_penjarahan'] ;
            $responce->rows[$i]['penjarahan']           = $result[$i]['penjarahan'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_penjarahan']);
//				$data['penjarahan'] = 
					
                $res = $this->db->insert("master_penjarahan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_penjarahan",$data['id_penjarahan']);
                $res = $this->db->update("master_penjarahan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_penjarahan",$id);
        $this->db->update("master_penjarahan",array("deleted"=>1));
    }
