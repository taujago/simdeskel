...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tanah Hutan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tanah Hutan'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tanah_hutan;
				console.log(row[i].id_tanah_hutan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
