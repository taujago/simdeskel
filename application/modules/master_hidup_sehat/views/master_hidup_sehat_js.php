...
function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Perilaku Hidup Bersih dan Sehat');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Perilaku Hidup Bersih dan Sehat'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_hidup_sehat;
				console.log(row[i].id_hidup_sehat);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
