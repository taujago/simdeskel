...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Tanah Basah');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Tanah Basah'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_tanah_basah;
				console.log(row[i].id_tanah_basah);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
