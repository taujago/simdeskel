<?php 
class profil_peraturan extends op_controller {
    var $desa2;
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("ppm","dm");
	...
	function get_data(){
       ...
       /*$nama_ibu = isset($_REQUEST['search_nama_ibu'])?$_REQUEST['search_nama_ibu']:"x";
        ...
        for($i=0; $i<count($result); $i++){
            ...
            $responce->rows[$i]['tentang']           = $result[$i]['tentang'] ;
            $responce->rows[$i]['persetujuan_bpd_no']           = $result[$i]['persetujuan_bpd_no'] ;
            $responce->rows[$i]['persetujuan_bpd_tgl']           = flipdate($result[$i]['persetujuan_bpd_tgl']) ;
            $responce->rows[$i]['dilaporkan_no']           = $result[$i]['dilaporkan_no'] ;
    ...
    function save(){
        ...
        $this->form_validation->set_rules('persetujuan_bpd_no','Nomor persetujuan BPD ','required');              
        $this->form_validation->set_rules('persetujuan_bpd_tgl','Tanggal persetujuan BPD  ','required');              
        $this->form_validation->set_rules('dilaporkan_no','Nomor dilaporkan ','required');              
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->insert("profil_peraturan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $res = $this->db->update("profil_peraturan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id",$id);
        $this->db->update("profil_peraturan",array("deleted"=>1));
    }
...
foreach($record->result() as $row) : 
    ...
    $this->excel->getActiveSheet()
        ...
        ->setCellValue('B' . $baris, "Nomor : $row->nomor Tanggal : $row->tanggal")
        ->setCellValue('C' . $baris, $row->tentang)
        ->setCellValue('D' . $baris, $row->uraian)
        ->setCellValue('E' . $baris, "Nomor : $row->persetujuan_bpd_no Tanggal : $row->persetujuan_bpd_tgl")
        ->setCellValue('F' . $baris,"Nomor :  $row->dilaporkan_no Tanggal : $row->dilaporkan_tgl");      
...
$baris+=3;
 $this->excel->getActiveSheet()         
        ...
        ->setCellValue('E' . $baris, date("d-m-Y"));
