...
$(".easyui-linkbutton").click(function(){
						$("#lembaga_diikuti").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Lembaga Pemerintahan yang Diikuti');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Lembaga Pemerintahan yang Diikuti'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_lembaga_diikuti;
				console.log(row[i].id_lembaga_diikuti);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
