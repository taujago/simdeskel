<?php 
class master_penculikan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("core_model","cm");
		$this->load->model("mpenc","dm");
	}


	function index(){
		$data['controller'] = get_class($this);
		$data['header'] = "Penculikan";
	...
	function get_data(){
        ...
        $sidx = isset($_REQUEST['sort'])?$_REQUEST['sort']:"penculikan"; // get index row - i.e. user click to sort 
        ...
        if($count == 0 ) {
        	$responce->rows['1']['id_penculikan']= "";
        }
        else {
                $x=0;
        for($i=0; $i<count($result); $i++){
        	$x++;
            //$responce->rows[$i]['id_penculikan']=$result[$i]['id_provinsi'];
            // data berikut harus sesuai dengan kolom-kolom yang ingin ditampilkan di view (js)
            $responce->rows[$i]['id_penculikan']			= $result[$i]['id_penculikan'] ;
            $responce->rows[$i]['penculikan']           = $result[$i]['penculikan'] ;
    ...
    function save(){
        ...
        if($this->form_validation->run() == TRUE) {
 				...
 				unset($data['id_penculikan']);
//				$data['penculikan'] = 
					
                $res = $this->db->insert("master_penculikan",$data);
    ...
    function update() {
        ...
        if($this->form_validation->run() == TRUE) {
                ...
                $this->db->where("id_penculikan",$data['id_penculikan']);
                $res = $this->db->update("master_penculikan",$data);
...
function hapus(){
    ...
    foreach($ids as $id) {
        $this->db->where("id_penculikan",$id);
        $this->db->update("master_penculikan",array("deleted"=>1));
    }
