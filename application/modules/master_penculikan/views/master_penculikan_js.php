...
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Penculikan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Penculikan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_penculikan;
				console.log(row[i].id_penculikan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
