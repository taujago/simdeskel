...
function baru(){
			mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Partai Politik');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Partai Politik'); 
	$('#fm').form('load',row);					 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_partai_politik;
				console.log(row[i].id_partai_politik);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
