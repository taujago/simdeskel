...
$(".easyui-linkbutton").click(function(){
						$("#kepemilikan_lahan").focus();
						});
function baru(){
		mode = "baru";
			$('#dlg').dialog('open').dialog('setTitle','Tambah Kepemilikan lahan pertanian tanaman pangan');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Kepemilikan lahan pertanian tanaman pangan'); 
	$('#fm').form('load',row);						 
...
function hapus() {
			...
			for(var i=0; i < row.length; i++) {
				arr[i] = row[i].id_kepemilikan_lahan;
				console.log(row[i].id_kepemilikan_lahan);
			}
			ids = arr.join();
			console.log(ids);
  			if (row){
