<?php 
class statistik_penduduk_lansia extends  op_controller {
 	function __construct() {
		parent::__construct();
 
		$this->load->model("stat_plansia","dm");
...
function index()
    ...
    $data['title'] = "Data Penduduk Lanjut Usia ";	
    $data['header'] = "Data Penduduk Lanjut Usia";
	$data['controller'] = "statistik_penduduk_lansia";
  

 
 
   	$content = $this->load->view("statistik_penduduk_lansia_view",$data,true);
	$this->set_title("Data Penduduk Lanjut Usia");
...
function get_data(){
 ...
 function cetak() {
	...
	$this->load->view("statistik_penduduk_lansia_cetak",$data); 	
...
function pdf(){
   $this->db->select('*')->from('v_penduduk p')
    ...
    $data['header'] = "DATA PENDUDUK USIA LANJUT ";
    $data['title'] = $data['header'];
        ...
        $pdf->AddPage('L');
         ...
         $html = $this->load->view("pdf/statistik_penduduk_lansia_pdf",$data,true);
         $pdf->writeHTML($html, true, false, true, false, '');
         ...
         if($halaman <> $pdf->getPage() ) {
            ...
            $pdf->AddPage();
            $html = $this->load->view("pdf/statistik_penduduk_lansia_pdf"."_table_header",$data,true);
            $pdf->writeHTML($html, true, false, true, false, '');
