...
$('#tanggal,#tanggal_surat,#search_tgl_awal').datebox({  
        required:false  ,
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Buku Ekspedisi');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Buku Ekspedisi '); 
	$('#fm').form('load',row);						 
