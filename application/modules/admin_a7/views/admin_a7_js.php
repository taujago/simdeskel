...
$('#tanggal, #masuk_tanggal, #keluar_tanggal').datebox({  
        required:false  ,
	...
	$('#search_tgl_akhir, #search_tgl_awal').datebox({  
        required:false  ,
        formatter : function(date) {
        	var y = date.getFullYear();
			var m = date.getMonth()+1;
...
function baru(){
			$('#dlg').dialog('open').dialog('setTitle','Tambah Data Agenda ');
			$('#fm').form('clear');
...
function edit() {
	...
	if(row) { 
	$('#dlg').dialog('open').dialog('setTitle','Edit Data Agenda '); 
	$('#fm').form('load',row);						 
	url = '<?php echo site_url("$controller/update") ?>';  
	
		if(row.jenis_surat == "keluar") {
			$(".keluar").show();
			$(".masuk").hide();
		}
		else {
...
function cari(){
	      ...
	      $('#tt').datagrid('load',{                         
 			...
 			search_jenis_surat : $("#search_jenis_surat").val(),
         	search_isi : $('#search_isi').val(),
...
function reset_search() {
	$("#search_jenis_surat").val('x').attr('selected','selected');
	$('#search_isi').val('');
...
function cetak(){
	...
	if (r){
		...
		window.open('<?php echo site_url("$controller/pdf") ?>/'+r);
	}
	});
}
